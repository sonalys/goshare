...
func Test_Ledger_Update(t *testing.T) {
	...
	t.Run("pass", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			setup(t, td, func(r ports.LocalRepositories) {
				td.ledger.Name = "updated name"

				err := r.Ledger().Update(ctx, td.ledger)
				require.NoError(t, err)

				updated, err := r.Ledger().Get(ctx, td.ledger.ID)
				require.NoError(t, err)
				assert.Equal(t, td.ledger, updated)
			})
		})
	})
	...
	t.Run("pass/delete member", func(t *testing.T) {
		ctx := t.Context()
		td := createTestData(t)

		setup(t, td, func(r ports.LocalRepositories) {
			td.ledger.Members = nil
			...
			updated, err := r.Ledger().Get(ctx, td.ledger.ID)
			require.NoError(t, err)
			assert.Empty(t, updated.Members)
		})
	})
}
